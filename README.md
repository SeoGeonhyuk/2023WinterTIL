...
1. 로드 밸런서를 통해서 트래픽 분산을 시도하려고 했으나 로드밸런서 자체에 과도한 트래픽이 몰려 로드밸런서가 과부화되는 문제
2. 로드밸런서의 수평확장을 통해서 해결
3. DNS에 로드밸런서들의 IP들을 등록해두면 DNS을 라운드로빈 방식을 사용하여 로드밸런서에 트래픽을 분산시킨다.

## 리해싱 문제

1. 데이터베이스 샤드가 추가되거나 제거될 때 레코드 이동으로 인해서 블로킹 또는 응답 시간 지연 문제가 발생하는 것.
2. 이를 해결하기 위해서 일관된 해시 알고리즘을 사용한다.
    1. https://uiandwe.tistory.com/1325
3. 또한 어떤 샤드에 해당 유저의 정보가 있는지 알려주는 매핑 DB 또한 설치하여 빠르게 특정 DB에 접근할 수 있다.⇒이렇게 과부화 될 우려가 있는 지점을 단일 실패 지점(SPOF, Single Point Of Failure)이라고 한다.
4. 매핑 DB 또한 과부화 될 우려가 존재할 경우 수평확장을 통해서 해결할 수 있다.

## 심리스 MMO 서버 구조

로직 서버는 일정 지역 안에 있는 캐릭터의 게임 로직 처리를 담당합니다. 그리고 게임 로직 처리를 하면서 변경되는 정보(위치, 체력 등)는 인접한 지역을 담당하는 다른 로직 서버에 지속적으로 복제합니다. 로직 서버가 담당하는 캐릭터 수가 지나치게 많아지면, 로직 서버가 담당하는 캐릭터의 소유권을 인접한 다른 로직 서버에 넘겨줍니다. 그리고 로직 서버가 담당하는 지역의 넓이를 줄입니다.(분산한다)

단점

1. 서버당 통신량과 OS 레벨 처리량이 많이 증가한다.
2. 따라서 서버 하나가 처리할 수 있는 클라이언트 개수가 상대적으로 적어지므로 서버 효율성이 떨어진다.
3. 같은 로직을 개발하더라도 해당 구조에서는 구현이 더 복잡할 수 있다.
4. 데이터를 옮기는 과정에서는 stale data problem 또는 블로킹 문제가 일어날 수도 있다.
